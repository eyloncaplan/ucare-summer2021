...
tPtHalf=np.linspace(0,T,2*t0+1)
            ...
            for l in range(max(0,math.floor(tPt[h]/ldt)-1),
                ...
                for e in range(2):
                    ...
                    # print("k=",k,", l=",l)
                        ...
                        #     curVal[0,k,l,j]+=ds/3*(baseEl(kdt,k,tPt[h]
                        #                                   +sPt[0,subInt])
                        ...
                        # RArray[2*h+1,k,l,j]=(basePsi[2*h-1,l]*curVal[0,k,l,j])
...
# def duCompSoln(t,dt,uVal,duVal):
#      k1=np.array()
...
#        print("h=",h)
        ...
        for i in range(N):
#            print("i=",i)
            uGamma2[:,i,i]=baseBias[2*h-1]
            uGamma3[:,i,i]=baseBias[2*h-1]
            uGamma4[:,i,i]=baseBias[2*h]
            uGammaArray[:,i,i]=baseBias[2*h]
            uTheta2[:,:,i,:,i]=RArray[2*h-1]
            uTheta3[:,:,i,:,i]=RArray[2*h-1]
            uTheta4[:,:,i,:,i]=RArray[2*h]
            uThetaArray[:,:,i,:,i]=RArray[2*h]
            kTheta1[:,:,i,:]=np.array([[[gDer(tPt[h]-dt,uArray[h-1],
 ...
 #       print("new kTheta1=",kTheta1)
        for l in range(max(0,math.floor((tPt[h]-.5*dt)/ldt)-1),
            ...
            for j in range(N):      
                uTheta2+=basePsi[2*h-1,l]*(yThetaArray[:,:,:,:,l,:,j]
                                   +.5*dt*kTheta1[:,:,:,:,l,:,j])
                uGamma2+=basePsi[2*h-1,l]*(yGammaArray[:,:,l,:,j]
                                   +.5*dt*kGamma1[:,:,l,:,j])
...
#                                  uTheta2[k,l,i,j],muVal)
 ...
 #               print("kGamma2=",kGamma2)
        for l in range(max(0,math.floor((tPt[h]-.5*dt)/ldt)-1),
            min(l0+1,math.ceil((tPt[h]-.5*dt)/ldt)+2)):
            for j in range(N):      
                uTheta3+=basePsi[2*h-1,l]*(yThetaArray[:,:,:,:,l,:,j]
                                   +.5*dt*kTheta2[:,:,:,:,l,:,j])
                uGamma3+=basePsi[2*h-1,l]*(yGammaArray[:,:,l,:,j]
                                   +.5*dt*kGamma2[:,:,l,:,j])
...
#                print("kGamma3=",kGamma3)
        for l in range(max(0,math.floor(tPt[h]/ldt)-1),
            min(l0+1,math.ceil(tPt[h]/ldt)+2)):
            for j in range(N):      
                uTheta4+=basePsi[2*h,l]*(yThetaArray[:,:,:,:,l,:,j]
                                  +dt*kTheta3[:,:,:,:,l,:,j])
                uGamma4+=basePsi[2*h,l]*(yGammaArray[:,:,l,:,j]
                                   +dt*kGamma3[:,:,l,:,j])
...
#        print("yTheta=",yThetaArray[:,:,:,:,1,:,1])
 ...
 #               print("[l,j]=",[l,j],", yGammaArray[l]=",yGammaArray[:,:,l,:,j])
                uThetaArray+=basePsi[2*h,l]*yThetaArray[:,:,:,:,l,:,j]
                uGammaArray+=basePsi[2*h,l]*yGammaArray[:,:,l,:,j]
...
#    print("uArray=",uArray)
 ...
 #       print("uSoln t=",t)
        ...
        for l in range(max(0,math.floor((t-.5*dt)/ldt)-1),
            ...
            for j in range(N):            
                u2+=basePsi[2*h-1,l]*(yArray[h-1,l,:,j]+.5*dt*k1[l,:,j])
        k2=g(t-.5*dt,u2,muVal)
        
        for l in range(max(0,math.floor((t-.5*dt)/ldt)-1),
                           min(l0+1,math.ceil((t-.5*dt)/ldt)+2)):
            for j in range(N):      
                u3+=basePsi[2*h-1,l]*(yArray[h-1,l,:,j]+.5*dt*k2[l,:,j])
        k3=g(t-.5*dt,u3,muVal)
        
        for l in range(max(0,math.floor(t/ldt)-1),
                           min(l0+1,math.ceil(t/ldt)+2)):
            for j in range(N):      
                u4+=basePsi[2*h,l]*(yArray[h-1,l,:,j]+dt*k3[l,:,j])
        k4=g(t,u4,muVal)
        yArray[h]=yArray[h-1]+1/6*dt*(k1+2*k2+2*k3+k4)
        
        for j in range (N):
            for l in range(l0+1):
                uArray[h]+=basePsi[2*h,l]*yArray[h,l,:,j]
        h+=1
    return uArray

basePhi=np.array([[baseEl(kdt,k,t) for k in range(k0+1)] for t in tPtHalf])
basePsi=np.array([[baseEl(ldt,l,t) for l in range(l0+1)] for t in tPtHalf])
baseBias=np.array([[baseEl(mdt,m,t) for m in range(m0+1)] for t in tPtHalf])
#print(dt,tPtHalf)
